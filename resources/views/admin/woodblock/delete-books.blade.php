					...
					<div class="grid gap-6 mb-8 md:grid-cols-2 xl:grid-cols-4">
						...
						<div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
							...
							<div>
								...
								<p class="text-lg font-semibold text-gray-700 dark:text-gray-200" id="total-books">
									{{ sizeof($books) }}
								</p>
							</div>
						</div>
									...
									<tr class="text-gray-700 dark:text-gray-400">
										...
										<td class="px-4 py-3">
											<div class="flex items-center space-x-4 text-sm">
												...
												<div class="btn-delete" id="{{ $book->id }}">
...
<script>
	...
	for (let i = 0; i < btnDeleteAll.length; ++i) {
		btnDeleteAll[i].addEventListener('click', function(e) {
			...
			$.ajax({
				type: 'DELETE',
				url: "/admin/books/list/" + code,
				data: obj,
				success: function(response) {
					console.log(response);
					alert('Xoá thành công!');
					btnDeleteAll[i].parentNode.parentNode.parentNode.style.display = 'None';
					let totalWoodblocks = parseInt(document.getElementById('total-books').innerText);
					document.getElementById('total-books').innerHTML = totalWoodblocks - 1;
				}
			});
